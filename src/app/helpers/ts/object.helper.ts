...
export class ObjectHelper {
    ...
    public static keys<TKey extends string>(obj: { [key in TKey]?: any }): Array<TKey> {
        return <Array<TKey>>Object.keys(obj);
    }

    public static values<TValue>(obj: { [key in string]?: TValue }): Array<TValue> {
    ...
    public static forEach<TKey extends string, TValue>(
        ...
        this.entries(object).forEach(([key, value], index: number, array: Array<[TKey, TValue]>) =>
    ...
    public static every<TKey extends string, TValue>(
        ...
        return this.entries(object).every(([key, value], index: number, array: Array<[TKey, TValue]>) =>
    ...
    public static some<TKey extends string, TValue>(
        ...
        return this.entries(object).some(([key, value], index: number, array: Array<[TKey, TValue]>) =>
            predicate(key, value, index, array)
        );
    }
    ...
    public static find<TKey extends string, TValue>(
        ...
        return this.entries(object).find(([key, value], index: number, array: Array<[TKey, TValue]>) =>
            predicate(key, value, index, array)
        );
    }
    ...
    public static deepCopy(object: DeepCopyable): DeepCopyable {
        ...
        if (object instanceof Array) {
            return ArrayHelper.clone(object).map(x => this.deepCopy(x));
        }

        // Object
        let copy: { [key: string]: any } = {};
        for (let key in object) {
            if (object.hasOwnProperty(key)) copy[key] = this.deepCopy(object[key]);
        }
        return copy;
    }
}
